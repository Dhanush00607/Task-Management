...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const displayTasks = (section, tasksToDisplay) => {
        ...
        taskContainer.addEventListener("click", (event) => {
            // Check if the clicked element has the class "bx-trash-alt"
            if (event.target.classList.contains("bx-trash-alt")) {
                // Retrieve the task ID from the closest ancestor with class "card"
                ...
                swal({
                    ...
                    if (willDelete) {
                        // Remove the task from localStorage using its ID
                        localStorage.removeItem(taskId);
    
                        // Remove the task's card from the DOM
